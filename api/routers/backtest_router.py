...
async def run_backtest_upload(
    ...
    long_only: bool = Form(True),          # match live DonchianLiveStrategy (#3)
    trend_ma: int = Form(200),
    fast_ma: int = Form(0),
    atr_period: int = Form(14),
    atr_multiplier: float = Form(1.5),
    use_atr_stop: bool = Form(True),
    volume_filter_days: int = Form(0),
    trailing_activation_pct: float = Form(1.0),
    trailing_pct: float = Form(8.0),
    starting_equity: float = Form(500000.0),
    slippage_bps: float = Form(0.0),
