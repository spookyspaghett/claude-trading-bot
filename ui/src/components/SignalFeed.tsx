import { Radio, Hourglass } from 'lucide-react'
...
const EVENT_STYLES: Record<string, { label: string; color: string; accent: string }> = {
  signal:          { label: 'SIGNAL',  color: 'text-blue-400 bg-blue-950 border-blue-800',       accent: 'border-l-blue-500/70' },
  order_submitted: { label: 'ORDER',   color: 'text-yellow-400 bg-yellow-950 border-yellow-800', accent: 'border-l-yellow-500/70' },
  fill:            { label: 'FILL',    color: 'text-green-400 bg-green-950 border-green-800',    accent: 'border-l-green-500/70' },
...
function EventRow({ evt }: { evt: LogEvent }) {
  ...
  return (
    <div className={`flex items-start gap-2.5 px-3 py-2 hover:bg-slate-800/40 transition-colors text-xs border-l-2 ${style.accent}`}>
  ...
  return (
    <div className="card flex flex-col overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between shrink-0">
        ...
        <span className={`flex items-center gap-1.5 text-xs font-medium ${wsConnected ? 'text-blue-400' : 'text-slate-600'}`}>
          <span className={`inline-block w-1.5 h-1.5 rounded-full ${wsConnected ? 'bg-blue-400 animate-pulse' : 'bg-slate-600'}`} />
